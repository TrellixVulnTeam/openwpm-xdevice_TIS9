...
# 		os.system('echo %s | sudo -S docker images' % (sudopass))
		...
		os.system('docker rm -f $(docker ps -a -q -f status=exited)')
		time.sleep(60)
...
def collect_ads(varlist):
	...
	os.system('docker rm $(docker ps --all -q -f status=exited)')
	time.sleep(60)
